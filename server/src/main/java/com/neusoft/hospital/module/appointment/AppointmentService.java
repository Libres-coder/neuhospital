...
public class AppointmentService {
    ...
    // ---------- recommend (very simple) ----------

    public List<DoctorDtoWithScore> recommend(String symptoms, String historyDept) {
        if (symptoms == null) symptoms = "";
        String sym = symptoms;
        String history = historyDept;
        return doctorRepository.findAll().stream().map(d -> {
            double expertise = (d.getExpertise() != null && d.getExpertise().contains(sym)) ? 0.5 : 0.2;
            double historyScore = (history != null && history.equals(d.getDepartmentId())) ? 0.3 : 0.1;
            double ratingScore = ((d.getRating() == null ? 4.5 : d.getRating()) / 5.0) * 0.2;
            double score = expertise + historyScore + ratingScore;
            return new DoctorDtoWithScore(toDoctorLite(d), (float) score);
